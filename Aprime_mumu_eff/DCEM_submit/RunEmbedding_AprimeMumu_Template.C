...
int RunEmbedding(
    ...
    const char* fn_sig = "/mnt/T2_US_MIT/hadoop/mitgroups/DarkQuest/wmccorma/Aprime_Muons_Mar31_DST/BREMVAL_DST.root",
    ...
    const char* fn_emb = "/mnt/T2_US_MIT/hadoop/mitgroups/DarkQuest/NUMBER/embedding_data.root",
    const int n_evt_in=10000)
{
  ///
  /// Global parameters
  ///
  ...
  Fun4AllDstOutputManager* man_out = new Fun4AllDstOutputManager("DSTOUT", out_path+"DST.root");
  ...
  /*
  int n_evt = n_evt_in == 0 || n_evt_in > n_evt_sig  ?  n_evt_sig  :  n_evt_in;
  */
