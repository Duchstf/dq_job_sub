...
def submit_job(config_file, config_dir):
    ...
    #Output files
    tag = "DCEM_Aprime_Electrons_Scan_z200_600" ####### FIRST PARAMETER
    tag += "_{}".format(now.strftime("%d_%m_%Y"))#Specify date and time

    macro_path = "/work/submit/dhoang/DQ/e1039-core/DarkQuest/e1039-analysis/SimHits/macro/RunEmbeddingNOEMBED_AprimeMumu_Template.C"
    macro_name = "RunEmbeddingNOEMBED_AprimeMumu_Template.C"
    ...
    macro_command = "'/tmp/$CONDOR_JOB_ID/RunEmbeddingNOEMBED.C({},{},{},{},10000)'"\
                    .format('"{}"'.format("/tmp/$CONDOR_JOB_ID/"+config_file+"_DST.root"),
                            '"{}"'.format("output.root"),
                            '"{}"'.format("/tmp/$CONDOR_JOB_ID/"),
                            '"{}"'.format("/mnt/T2_US_MIT/hadoop/mitgroups/DarkQuest/NUMBER/embedding_data.root"))
...
def main():
    ...
    dst_dir = "/mnt/T2_US_MIT/hadoop/mitgroups/DarkQuest/dhoang/Aprime_Electrons_Scan_z200_600_20_04_2022/"
    ...
    for filename in os.listdir(dst_dir):
        ...
        # if test_count == 5: 
        #     break    
    print("Finished processing {} Files".format(count_file))
